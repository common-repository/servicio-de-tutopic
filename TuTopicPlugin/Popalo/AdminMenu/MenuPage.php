...
class MenuPage {
    ...
    public function __construct($args = []) {
      ...
      if (isset($args['notification_count']))
        $notificationsCount = $args['notification_count'];

      $title = $notificationsCount ?
                sprintf($this->menu_title.' <span class="awaiting-mod">%d</span>', $notificationsCount)
                : $this->menu_title;

      // Agregar pagina de menu
      $menu = add_menu_page($this->title, $title, $this->capability, $this->menu_slug, $this->callback, $this->icon_url, $this->position );

      if ($cb = $this->styles_callback) {
        add_action( 'admin_print_styles-' . $menu,  $cb );
      }
      if ($cb = $this->scripts_callback) {
        add_action( 'admin_print_scripts-' . $menu, $cb );
      }
